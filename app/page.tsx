...
import { 
  ...
  Heart, PhoneCall, Star, TrendingUp, Save, Cat
} from "lucide-react";
...
const SakuraCSS = () => (
  <style jsx global>{`
    ...
    .curvy-font { font-family: 'Fredoka', cursive; }
...
export default function Home() {
  ...
  async function saveCallLog() {
    const { error } = await supabase.from("call_history_logs").insert([{ call_count: dailyCalls, csat_score: parseFloat(dailyCSAT) }]);
  ...
  return (
    <main className="min-h-screen bg-[#fff5f7] curvy-font pb-24 overflow-x-hidden text-gray-800 tracking-tight">
      ...
      <div className="relative z-10 bg-white/40 backdrop-blur-xl border-b border-pink-200 p-6 mb-6 text-center shadow-sm">
        <h1 className="text-xl sm:text-2xl font-black text-pink-600 italic uppercase tracking-tighter">
      ...
      <div className="max-w-4xl mx-auto px-4 relative z-10">
        <div className="flex bg-white/30 backdrop-blur-md p-1.5 rounded-2xl mb-8 border border-white/40 overflow-x-auto no-scrollbar">
          {['dashboard', 'ledger', 'calls', 'analytics', 'tips'].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 min-w-[85px] py-2.5 text-[9px] font-black rounded-xl capitalize transition-all ${activeTab === tab ? 'bg-white text-pink-600 shadow-lg pink-glow' : 'text-pink-400'}`}>{tab}</button>
          ))}
        </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="glass-card p-6 text-center text-gray-800">
              ...
              <div className="mt-6">
                <div className="flex justify-between text-[9px] font-black uppercase mb-1"><span>Monthly Goal Progress</span><span>{totalPoints} / {monthlyGoal} PTS</span></div>
            ...
            <div className="glass-card p-6">
              ...
              <div className="space-y-4">
                ...
                <button onClick={addSale} className="w-full bg-pink-500 text-white p-5 rounded-2xl font-black shadow-lg pink-glow uppercase tracking-widest text-[10px] active:scale-95">Log Points Milestone 🌸</button>
              </div>
            </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-5">
            <div className="glass-card p-6 text-center">
              ...
              <div className="mt-8 border-t border-pink-100 pt-6 text-left">
                ...
                <button onClick={saveCallLog} className="w-full bg-pink-600 text-white p-4 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 pink-glow shadow-md"><Save size={14}/> Save Current Call Stats</button>
              </div>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
              {[{l:'Yesterday',v:getFilteredCalls(1)},{l:'7 Days',v:getFilteredCalls(7)},{l:'15 Days',v:getFilteredCalls(15)},{l:'1 Month',v:getFilteredCalls(30)}].map((s,i)=>(
            ...
            <div className="glass-card p-6">
              ...
              <div className="divide-y divide-pink-100 max-h-[40vh] overflow-y-auto no-scrollbar text-gray-700">
                {callHistory.map(log => (
                  <div key={log.id} className="py-3 flex justify-between items-center group">
                    <div>
                      <p className="font-bold text-sm leading-none">{log.call_count} Calls • {log.csat_score}% CSAT</p>
                      <p className="text-[9px] text-pink-400 font-bold uppercase italic mt-1.5">{new Date(log.created_at).toLocaleString()}</p>
                    </div>
              ...
              {CATEGORIES.map(cat => {
                ...
                return (
                  <div key={cat} className="space-y-2">
                    <div className="flex justify-between items-center text-[9px] font-black uppercase"><span>{cat}</span><span>{perc}%</span></div>
                    <div className="h-2.5 bg-pink-100/30 rounded-full overflow-hidden border border-white"><div className="h-full bg-gradient-to-r from-pink-300 to-pink-500 transition-all duration-1000" style={{ width: `${perc}%` }}></div></div>
                  </div>
                );
              })}
            </div>
        ...
        {activeTab === 'tips' && (
          <div className="space-y-4 animate-in duration-500 text-gray-600 font-semibold">
            ...
            <div className="glass-card p-6 rounded-[2.5rem]">
              {activeTipStage === 'seed' && (
                <div className="space-y-4">
                  <h3 className="text-pink-600 font-black text-xs uppercase text-center mb-4 tracking-widest italic leading-none">Planting the Seed 🌱</h3>
                  {[
                    ...
                    "Before we dive into resolving your main concern today, I just noticed while pulling up your file that your account is fully eligible for a brand-new exclusive offer. Let's take a look after we get this current issue sorted out for you!",
                  ...
                  ].map((t, i) => <div key={i} className="p-4 bg-white/50 rounded-2xl border-l-4 border-pink-400 italic text-xs leading-relaxed font-semibold shadow-sm italic tracking-tight">{"\""}{t}{"\""}</div>)}
                </div>
              )}
              ...
              {activeTipStage === 'objection' && (
                <div className="space-y-4 animate-in fade-in">
                  ...
                  ].map((item, i) => <div key={i} className="p-4 bg-white/50 rounded-2xl border-l-4 border-pink-400 shadow-sm space-y-1 italic tracking-tight text-xs"><p className="text-[9px] font-black text-pink-600 uppercase underline">Objection: {"\""}{item.o}{"\""}</p><p>What to say: {"\""}{item.s}{"\""}</p></div>)}
                </div>
              )}
              {activeTipStage === 'close' && (
                <div className="space-y-4 animate-in fade-in">
                  <h3 className="text-pink-600 font-black text-xs uppercase text-center mb-4 tracking-widest leading-none">Closing the Deal 🏆</h3>
                  {[
                    "Awesome! Let's get that added right now so you don't miss out on today's discount. Would you prefer confirmation via email or text message?",
                    "Recap: you're getting faster service and a lower bill. I'm just sending a quick confirmation text to your phone now—sound good?"
                  ].map((t, i) => <div key={i} className="p-4 bg-white/50 rounded-2xl border-l-4 border-pink-400 italic text-xs leading-relaxed font-semibold shadow-sm italic tracking-tight">{"\""}{t}{"\""}</div>)}
                </div>
              )}
            </div>
